<?php
    ...
    Plugin URI: https://www.powr.io/plugins/video-gallery
    ...
    */
    ...
    class Powr_Video_Gallery extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Video Gallery.
        </p>
        <p>
    ...
    function powr_video_gallery_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-video-gallery' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-video-gallery' label='$label'></div>";
      }else{
      	"<div class='powr-video-gallery'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_video_gallery_menu' );
    function powr_video_gallery_menu() {
      add_menu_page( 'POWr Video Gallery', 'POWr Video Gallery', 'manage_options', 'powr-video-gallery-settings', 'powr_video_gallery_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_video_gallery_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/video-gallery';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/video-gallery';
      }
    ...
    register_activation_hook(__FILE__, 'powr_video_gallery_plugin_activate');
      add_action('admin_init', 'powr_video_gallery_plugin_redirect');

      function powr_video_gallery_plugin_activate() {
      add_option('powr_video_gallery_plugin_do_activation_redirect', true);
      }
      ...
      function powr_video_gallery_plugin_redirect() {
      if (get_option('powr_video_gallery_plugin_do_activation_redirect', false)) {
          delete_option('powr_video_gallery_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-video-gallery-settings&' );
          }
       }
      }
